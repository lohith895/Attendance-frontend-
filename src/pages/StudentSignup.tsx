...
export default function StudentSignup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.fullName || !formData.rollNumber || !formData.sectionId || !formData.password || !formData.email || !formData.phoneNumber) {
    ...
    if (capturedImages.length < 5) {
      toast({ title: "Face Images Required", description: "Please capture at least 5 face images for registration.", variant: "destructive" });
      return;
    }
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="grid gap-6 md:grid-cols-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label>Email *</Label>
                    <Input type="email" placeholder={`${formData.rollNumber || 'roll'}@attendance.edu`} value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} required />
                  </div>
                  <div className="space-y-2">
                    <Label>WhatsApp Number *</Label>
                    <Input type="tel" placeholder="919876543210" value={formData.phoneNumber} onChange={e => setFormData({ ...formData, phoneNumber: e.target.value })} required />
                  </div>
                  <div className="space-y-2">
                ...
                {/* Face Capture */}
                <div className="space-y-4">
                  <h3 className="font-semibold flex items-center gap-2"><Camera className="h-4 w-4" /> Face Training *</h3>
