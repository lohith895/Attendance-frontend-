  ...
  const handleWhatsappAlert = () => {
    ...
    const links = lowStudents.map((s) => {
      const message = encodeURIComponent(
        `Dear ${s.fullName} (${s.rollNumber}), your attendance is ${s.percentage}% which is below 80%. Please attend classes regularly to avoid academic issues. - ${APP_SHORT_TITLE}`
      );
