...
export default function TakeAttendance() {
  ...
  const downloadAttendance = async (classId: string) => {
    const classInfo = todaysClasses.find(c => c.id === classId);
    if (!classInfo) return;
    ...
    if (error || !records) {
      toast({ title: "Download Failed", description: "Could not fetch attendance records.", variant: "destructive" });
      return;
    }

    // Build CSV
    const headers = ["Roll Number", "Student Name", "Email", "Status", "Confidence", "Manual Override", "Marked At"];
    const rows = records.map((r: any) => {
      ...
      return [
        ...
        student?.email || "",
        r.status,
        r.face_confidence != null ? `${Math.round(r.face_confidence * 100)}%` : "N/A",
    ...
    a.href = url;
    a.download = `attendance_${classInfo.subject_code}_${classInfo.start_time}_${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
          ...
          <div className="lg:col-span-2 space-y-4">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {/* Download button for completed classes */}
                {selectedClassInfo?.status === "completed" && (
                  <Button
                    variant="outline"
                    onClick={() => downloadAttendance(selectedClass)}
                    className="w-full gap-2"
                  >
